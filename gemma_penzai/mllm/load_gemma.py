...
Gemma3SigLipConfig = siglip.Gemma3SigLipConfig
SigLiPFromPatches = siglip.SigLiPFromPatches
...
def gemma_from_pretrained_checkpoint(
  ...
  * Axes of parameters are identified by name instead of by position.
  ...
  for i in range(config.num_decoder_blocks):
    ...
    if config.use_qk_norm:  # pytype: disable=attribute-error
...
def gemma_multimodal_from_pretrained_checkpoint(
    ckpt_params: dict[str, Any],
    upcast_activations_to_float32: bool = False,
    use_layer_stack: bool = False,
    preset_name: Literal[
...
) -> gemma_multimodal.MultiModalTransformerLM:
  """Builds a Gemma multimodal model from a pretrained checkpoint.
  ...
  * Axes of parameters are identified by name instead of by position.
  ...
  Args:
    ...
    preset_name: Preset name, used to determine model config. If "auto", uses
      the number of layers and whether the model needs qk norm in the checkpoint
      to determine the configuration.
    text_only: Whether to use the text-only version of the model.
  ...
  config = gemma_multimodal.GemmaTransformerConfig(
      **preset_kwargs,
      parameter_dtype=parameter_dtype,
      mlp_variant="geglu_approx",
      tie_embedder_and_logits=True,
  ...
  if not text_only:
    assert _HAS_VISION_ENCODER[
        preset_name
    ], f"Vision encoder is not supported for this preset {preset_name}."
    vision_config = Gemma3SigLipConfig()
  else:
    vision_config = None
  model_def = gemma_multimodal.build_gemma_multimodal_transformer(
      config,
      init_base_rng=None,
      name="transformer",
  ...
  if not text_only:
    # load vision projection parameters
    parameter_mapping["mm_input_projection.weights"] = pz.nx.NamedArray.wrap(
        params["embedder/mm_input_projection"]["w"]
    ).tag("vision_embedding", "embedding")
    parameter_mapping["mm_soft_embedding_norm/scale.weights"] = (
        pz.nx.NamedArray.wrap(
            1 + params["embedder/mm_soft_embedding_norm"]["scale"]
        ).tag("embedding")
    )
    ...
    parameter_mapping["vision_encoder/final_norm/scale.weights"] = (
        pz.nx.NamedArray.wrap(
            vision_encoder_params["encoder_norm"]["scale"]
        ).tag("embedding")
    )
    parameter_mapping["vision_encoder/final_norm/shift.bias"] = (
        pz.nx.NamedArray.wrap(
            vision_encoder_params["encoder_norm"]["bias"]
        ).tag("embedding")
    )
    ...
    for i in range(vision_config.num_encoder_blocks):
      ...
      parameter_mapping[
          ...
          vision_encoder_params[f"encoderblock_{i}/MlpBlock_0/Dense_0"][
              "kernel"
          ]
      ).tag(
          "embedding",
      ...
      parameter_mapping[
          ...
          vision_encoder_params[f"encoderblock_{i}/MlpBlock_0/Dense_1"][
              "kernel"
          ]
      ).tag(
          "neurons",
      ...
      parameter_mapping[
          f"vision_encoder/block_{i}/attention/output.weights"
      ] = pz.nx.NamedArray.wrap(
          vision_encoder_params[
